...
def infinite_albums():
    ...
    # Vilken rad i tabellen som SQL-query startar på
    offset = int(request.form.get("get_albums"))
    # Follow-flöde eller explore-flöde
    flow_type = int(request.form.get("flow_type"))
    if flow_type == 1:
...
def upload():
    ...
    if(request.form.get("album_id")):
        ...
        if(not owns_album(album_id, username=session["username"])):
            return jsonify(False), 413, {"ContentType": "application/json"}
        cur.execute("update album set country=%s, city=%s, date_start=%s, date_end=%s where id=%s",
                    (country, city, date_start, date_start, album_id))
        ...
        cur.execute("insert into album(owner, published, country, city, date_start, date_end) values(%s, %s, %s, %s, %s, %s) returning id",
                    (user_id, datetime.utcnow(), country, city, date_start, date_end))
    ...
    for key in request.files:
        ...
        cur.execute("insert into post(album, index, img_name, headline, description) values(%s, %s, %s, %s, %s)",
                    (album_id, index, filename, headline, description))
    db.conn.commit()
...
def edit_album(album_id):
    ...
    cur.execute(
        "select country, city, date_start, date_end, id from album where id=%s", (album_id,))
...
def album(album_id):
    ...
    # Hämtar information om alla bilder
    cur.execute(
        "select img_name, headline, description, index from post where album={} order by index asc".format(album_id))
    posts = cur.fetchall()

    is_own_album = "username" in session and get_user_id(
...
def upload_profile_img():
    ...
    cur.execute("update person set profile_img=%s where username=%s", [
                secure_filename(filename), session["username"]])
    db.conn.commit()
    cur.close()
...
def upload_background_img():
    ...
    # Uppdaterar profile_img till filnamnet bilden fick
    cur.execute("update person set background_img=%s where username=%s", [
                secure_filename(filename), session["username"]])
    db.conn.commit()
    cur.close()
    return jsonify(filename), 200, {'ContentType': 'application/json'}
